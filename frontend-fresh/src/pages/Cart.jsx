...
//                       style={{
//                         width: "00px",      // forced width (passport-like)
...
function buildImageSrc(imgPath) {
  if (!imgPath) return ""; // no image
  // If imgPath already looks like a full URL (http/https) return as-is
  if (/^https?:\/\//i.test(imgPath)) return imgPath;
  // If it already starts with a slash, just prefix the API origin
  if (imgPath.startsWith("/")) return `${API}${imgPath}`;
...
export default function Cart() {
  ...
  const updateQty = (id, qty) => {
    dispatch({ type: "UPDATE_QTY", id, qty: Math.max(1, Number(qty) || 1) });
  };
  ...
  const handleCheckout = () => {
    // If you have auth, you can redirect to checkout route
    navigate("/checkout");
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">Add items to your cart to see them here.</p>
          <Link to="/" className="inline-block bg-yellow-500 text-black px-4 py-2 rounded">
            Continue shopping
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {cart.map((item) => (
            ...
            >
              {/* Image: centered vertically + fixed passport-like size */}
              <div className="flex-shrink-0 w-[100px] h-[200px] bg-gray-700 rounded overflow-hidden flex items-center justify-center">
                {item.image ? (
                  <img
                  ...
                  <div className="text-xs text-gray-300 px-2 text-center">No image</div>
                )}
              </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="ml-4 text-sm px-3 py-1 bg-red-600 rounded hover:bg-red-700"
                  >
                    Remove
                  </button>
                </div>
              ...
              <div className="text-right">
                <div className="text-sm text-gray-400">Item total</div>
                <div className="font-bold text-lg">₹{Number(item.price || 0) * Number(item.qty || 1)}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 bg-gray-800 p-4 rounded-lg flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-2 bg-yellow-500 text-black rounded font-semibold hover:bg-yellow-600"
            >
              Place Order
            </button>
          </div>
